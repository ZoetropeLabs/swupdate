...
static EVP_PKEY *load_pubkey(const char *filename)
{
    ...
    ERR_clear_error();
    if (!PEM_read_bio_RSAPublicKey(key_bio, &rsa_pkey, NULL, NULL))
    {
	       	...
	       	const char *file)
{
	...
	if(siglen <= 0) {
		ERROR("Error getting maximum size of signature from public key (got %d) - err %ld\n", siglen, ERR_peek_last_error());
		status = -ENOKEY;
		goto out;
	}
